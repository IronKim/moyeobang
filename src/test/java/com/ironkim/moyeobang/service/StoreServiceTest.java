...
import com.ironkim.moyeobang.repository.*;
...
class StoreServiceTest {
        ...
        void 스토어등록이_정상동작한다() {
                ...
                org.mockito.Mockito.verify(storeContactRepository, times(1)).save(any(StoreContact.class));
        }
        ...
        void 스토어_상세_조회_성공() {
                Long storeId = 1L;
                Account account = createAccount("owner_escape");
                Store store = Store.builder()
                                ...
                                .authStatus(AuthStatus.PENDING)
                                .build();
                store.getContactList().add(StoreContact.builder()
                                .id(10L)
                                .store(store)
                ...
                var response = sut.getStoreDetail(storeId);

                assertThat(response.getStoreId()).isEqualTo(storeId);
                assertThat(response.getBusinessName()).isEqualTo("미로연구소");
                assertThat(response.getLatitude()).isEqualByComparingTo(new BigDecimal("37.556993"));
                assertThat(response.getStoreNumberList()).hasSize(1);
                assertThat(response.getStoreNumberList().get(0).getStoreNumber()).isEqualTo("01012345678");
        }

        @Test
        void 스토어_상세_조회_시_스토어가_없으면_예외가_발생한다() {
                Long storeId = 999L;

                when(storeRepository.findById(storeId)).thenReturn(Optional.empty());

                MoyeobangApplicationException e = assertThrows(MoyeobangApplicationException.class,
                                () -> sut.getStoreDetail(storeId));
        ...
        void 스토어_수정_성공() {
                ...
                StoreUpdateRequest updateRequest = StoreUpdateRequest
                                .builder()
        ...
        @Test
        void 스토어_수정_시_권한이_없으면_예외가_발생한다() {
                Long storeId = 1L;
                String otherAccountId = "other_account";
                Account account = createAccount("owner_escape");
                Store store = Store.builder()
                                .id(storeId)
                                .account(account)
