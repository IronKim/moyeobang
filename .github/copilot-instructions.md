...
- CBD 방법론으로 컴포넌트를 작성한다.
- 컴포넌트는 단일 책임 원칙을 지키며 최대한 작게 유지한다.
...
- 이벤트 핸들러는 onInput, onClick 등 명확한 네이밍을 유지한다.
- 상태 업데이트는 setState(prev => ...) 패턴으로 안전하게 처리한다.
- 반복되는 로직은 커스텀 훅(useXXX)으로 분리한다.
